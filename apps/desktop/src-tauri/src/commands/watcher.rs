...
use crate::db::{SqliteRepository, WatcherRepository};
...
pub async fn start_watching(
    ...
    // Persist to DB only after watcher is confirmed active
    {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            repo.add_watched_directory(&dir_path_clone).map_err(|e| e.to_string())
        })
        .await
    ...
    scan_directory(path, app_handle, state.repository.clone()).await;
...
pub(crate) async fn scan_directory(
    ...
    let deck_paths = tokio::task::spawn_blocking(move || {
        let mut paths: HashSet<String> = HashSet::new();
        scan_directory_recursive(&dir, &repository, &mut paths);
        paths
    })
    .await
    .unwrap_or_default();
...
fn scan_directory_recursive(dir: &PathBuf, repository: &Arc<Mutex<SqliteRepository>>, deck_paths: &mut HashSet<String>) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            scan_directory_recursive(&path, repository, deck_paths);
            ...
            if let Ok(content) = std::fs::read_to_string(&path) {
                if let Ok(cards) = flashcard_core::parser::parse(&content) {
                    // Import with lock, then release before writing back
                    let diff_result = {
                        let repo = match repository.lock() {
                            Ok(r) => r,
                            Err(_) => continue,
                        };
                        repo.import_cards(&deck_path, &source_file, &cards).ok()
                    };
                    // Lock dropped here
                    if let Some(diff) = diff_result {
                        if !diff.id_assignments.is_empty() {
                            let updated = flashcard_core::parser::inject_ids(&content, &diff.id_assignments);
...
pub async fn refresh_watched_directories(
    ...
    for dir_str in &dirs {
        ...
        if path.is_dir() {
            scan_directory(path, app_handle.clone(), state.repository.clone()).await;
        }
    }
